...
def discriminator(image, trainable=True):
    ...
    A1 = tf.contrib.layers.conv2d(image, 64, kernel_size=5, padding="SAME", stride=[2,2], trainable=trainable, normalizer_fn=tf.contrib.layers.batch_norm, activation_fn=tf.nn.relu, normalizer_params = {"decay" : 0.95})
    A2 = tf.contrib.layers.conv2d(A1, 128, kernel_size=5, padding="SAME", stride=[2,2], trainable=trainable, normalizer_fn=tf.contrib.layers.batch_norm, activation_fn=tf.nn.relu, normalizer_params = {"decay" : 0.95})
    A3 = tf.contrib.layers.conv2d(A2, 256, kernel_size=5, padding="SAME", stride=[2,2], trainable=trainable, normalizer_fn=tf.contrib.layers.batch_norm, activation_fn=tf.nn.relu, normalizer_params = {"decay" : 0.95})
    Z4 = tf.layers.dense(tf.reshape(A3, [minibatch_size, -1]), 4*4*256, trainable=trainable)
    return Z4
...
def generator(z):
    ...
    X_G = tf.contrib.layers.batch_norm(tf.nn.conv2d_transpose(A3, G_W4, [minibatch_size, 32, 32, 3], strides=[1, 2, 2, 1]), activation_fn=tf.nn.tanh, decay=0.95)
    return X_G
...
with tf.Session() as sess:
	...
	D_fake_logit = discriminator(G_images, trainable=False) #Won't update the Discriminator Network Weights while doing BackProp, only Generator weights are updated
	...
	for epoch in range(num_training_iterations):
		...
		ims(str(epoch)+".jpg",merge(sdata[0],[8,8]))
